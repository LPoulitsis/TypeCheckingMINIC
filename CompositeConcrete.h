...
public:
	...
	ValueStruct setValue(double); // new method
	void setType(Type*); // new method
	ValueStruct getValue();
	Type* getType();
	virtual ~CExpression() {}
	
protected:
	// type and value members
	Type* type;
	ValueStruct val;
...
public:
	...
	CExpressionNUMBER(const char* text, double value, TypeEnum type);
	virtual ~CExpressionNUMBER() {}
	virtual void PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) override;
	Type* GetTypeOfNumber(TypeEnum); // method returning type
	ValueStruct Evaluate(CSTNode* parent) override;
...
public:
	...
	virtual void PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) override;
	ValueStruct Evaluate(CSTNode* parent) override;
	ValueStruct GetActualArgument(int index);
